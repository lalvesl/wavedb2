...
///
/// Planning two tree mutations on the **same** tree inside one atomic batch
/// (a `save`'s old-key removal, then its new-key insert) must let the second
/// plan read the first's node writes — otherwise both rewrite the same node
/// from its original state and the later write silently undoes the earlier.
