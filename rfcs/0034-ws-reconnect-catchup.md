...
> (fmt + clippy + tests + budget). **Follow-up hardening (optional):** a
> real-node two-process restart e2e (extend `live_watch_e2e.rs` with a node
> kill/restart mid-watch) would mirror `live_watch_poll_e2e.rs` for the push
> path — the unit-level loopback proof already covers the mechanism.
