...
- **Status:** In progress (WIP) — M7 W8, the last M7 piece
...
>
> **Remaining:** NonUnique `insert`/`update`/`remove` (an offline `insert`'s id
> is node-minted — needs a provisional-id + reconciliation model), a durable
> queue across restarts, and an app-facing conflict surface.
