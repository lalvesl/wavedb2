  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        guiLibs = import ./nix/gui-libs.nix pkgs;

        # ── the benchmark suite (benches/nix/) ────────────────────────────────
        bench = import ./benches/nix {
          inherit pkgs rustPlatform rustToolchain;
          repoSrc = ./.;
        };
      ...
      in
      {
        ...
        # ── bench (benches/nix) ───────────────────────────────────────────────
        #
        # Declared here one by one rather than merged wholesale, so `nix flake
        # show` and a grep for an output name both still find it in this file.
        packages.bench-gen = bench.packages.bench-gen;
        packages.bench-dataset = bench.packages.bench-dataset;
        packages.bench-seed-wavedb = bench.packages.bench-seed-wavedb;
        packages.bench-seed-sqlite = bench.packages.bench-seed-sqlite;
        packages.bench-seed-postgres = bench.packages.bench-seed-postgres;
        packages.bench-seed-mysql = bench.packages.bench-seed-mysql;
        packages.bench-seed-mongodb = bench.packages.bench-seed-mongodb;

        apps.bench = bench.apps.bench;
        apps.bench-seeded = bench.apps.bench-seeded;
